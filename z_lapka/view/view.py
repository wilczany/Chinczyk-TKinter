...
class GameView:
    ...
    # def set_moves_board(self, possible_moves: []):
    #     for move_index, tile in enumerate(possible_moves):
    #         self.track[tile].configure(command=lambda: self.move_pawn(move_index))


    #
    #   Controlling buttons in board
    #   
    ...
    def unlock_home(self, tile, color):
        self.home_lines[color][tile].configure(command=lambda: self.move_to_home(color, tile))
    ...
    def roll_dice(self):
        ...
        if moves == None:
            ...
            print("ilosc ruchow:" + str(len(moves)))
    ...
    def move_from_base(self, color):
        #print("move from base" + str(color))
        next_pos = self.game.execute_move(-1, self.dice_value, color)
        self.next_player_turn()
        self.track[next_pos].configure(image=self.pawns[color])    
        # match color:
        #     case 0:
        #         self.track[0].configure(image=self.pawns[self.player_turn])
        #     case 1:
        #         self.track[10].configure(image=self.pawns[self.player_turn])
        #     case 2:
        #         self.track[20].configure(image=self.pawns[self.player_turn])
        #     case 3:
        #         self.track[30].configure(image=self.pawns[self.player_turn])
    ...
    def next_player_turn(self):
        ...
        self.message_box.configure(text="Ruch gracza: " + self.kolory[self.player_turn])
        self.message_box.after(1000, self.unlock_dice)
